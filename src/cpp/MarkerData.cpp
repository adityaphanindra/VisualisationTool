...
Marker::Position Marker::getCentre(vector<Marker::MarkerData> markers)
{
    ...
    for(Marker::MarkerData current : markers)
    {
        centre.x += current.getPosition().x;
        centre.y += current.getPosition().y;
        centre.z += current.getPosition().z;
    }

    centre.x /= markers.size();
    centre.y /= markers.size();
    centre.z /= markers.size();
